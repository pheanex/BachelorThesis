...
def update_from_device():
        global graph
        graph = pydot.Dot(graph_type='graph', layout="dot", sep="+20")
        ...
        for ap_name, ap_lan_mac, dest_ap_bssid, channel, signal_strength in scan_results:
                ...
                if list_of_ap_names_of_seen_node:
                        ...
                        #create the edge
                        graph.add_edge(pydot.Edge(ap_name, list_of_ap_names_of_seen_node[0], style="dotted", label=str(channel), fontsize="10", penwidth=str(int(signal_strength) / 20.0), color="grey50", weight=str(signal_strength)))
                                                ...
                                                if slave != "None":
                                                        if master != "None":
                                                                if status == "1":
                                                                        ...
                                                                        graph.add_edge(pydot.Edge(slave, master, penwidth="2", color="green"))
                                                                else:
                                                                        if debug:
