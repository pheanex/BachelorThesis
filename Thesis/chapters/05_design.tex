...
\section{Topology Creation}
  ...
  $$ES=\frac{bw}{(im + 1 )* cc}$$
  where \textit{ES} is the Score of this Edge, \textit{bw} is the expected bandwidth, \textit{im} is the number of interfering modules and \textit{cc} is the connected count of the corresponding nodes.
  \begin{description}
    ...
    Granting this value a higher impact in the formula, like for example in  $$ES=\frac{bw}{(im + 1 )* cc^2}$$, would have the effect of overemphasizing 
