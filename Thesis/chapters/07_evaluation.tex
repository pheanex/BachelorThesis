...
\newpage
  ...
  \section{Preparation}
    ...
    therefore mac- address and table. Cabling the clients to the AP instead of using real mobile devices like smartphones was preferred as this gave
...
\newpage
    ...
    \subsection{Logical Structure}
      ...
      The VLAN infrastructure is completely transparent to the VZ containers and the APs, since the host-machine on the server 
      encapsulates all frames before sending it over the VLAN Trunk to the switch, which again decapsulates the frames before sending them to the APs.
	...
	\label{fig:testsetup_openvz}
      ...
      1Mbit * 12 * 2 = 24Mbit/s for each AP. As we will see later on, this will turn out to be enough to completely saturate the wireless network but is still within
...
\newpage
    ...
    setup we recognize noticeable differences for each channel-assigning.
      ...
      \begin{itemize}
	\item The base scenarios where only one channel was utilized performe the worst within a range of 0.5 to 1.5Mbit/s on average per AP.
...
\newpage
    ...
    successfully and all transmitted packets we can now take a look at the effective rate of corrupt packets and therefore bytes, as an error in a packet 
	...
	we discern greater deviations especially for those tests in the 5Ghz band.
    \end{itemize}
  ...
  \section{Analysis}
    ...
    decrease overall throughput down to about 100kbit or less, growing worse with each newly added AP in receive-range. 
    ...
    limits us in selecting separate modules for different connections so that an AP which already established two connections over its two modules
	...
	\caption{Link separation capability for an AP with three modules compared to an APs with two modules.}
      }
    ...
    Of course this effect is limited by using a separate channel for each link and also by the number of modules for an AP 
	...
	\item[Multiple Radios Utilized:]
	  ...
	  \begin{figure}[h!]
	    \centerline{
	      ...
	      \caption{Unused modules can be used for separate client connections without interfering the WLAN backbone.}
	    }
