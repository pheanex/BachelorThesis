...
class Counter(dict):
    ...
    def most_common_all(self):
        '''List all the most common elements and their counts.

        >>> Counter('aaabbbccdd').least_common_all()
        [('a', 3),('b', 3)]
    ...
    def least_common(self, n=None):
        ...
        >>> Counter('abcdeabcdabcaba').least_common(3)
        [('c', 3), ('b', 4), ('a', 5)]

        '''
        # Emulate Bag.sortedByCount from Smalltalk
        if n is None:
            return sorted(self.iteritems(), key=_itemgetter(1), reverse=False)
        return _heapq.nsmallest(n, self.iteritems(), key=_itemgetter(1))

    def least_common_all(self):
        '''List all the least common elements and their counts.
