...
if __name__=='__main__':			
	...
	logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', filename=os.path.join(dbpath,'Py27-genres-IperfUDP.log'))
	...
	if len(sys.argv)>1:
		...
		resulttext.extend(my_test.csv_add_extract(os.path.join(testpath,'Results.csv'), testrun.get_empty_result(), {'columnsversus': [(1,0), (2,0), (3,0), (4,0)]})) # <== add extracts of csvfile
