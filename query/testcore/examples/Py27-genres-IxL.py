...
if __name__=='__main__':
	...
	test_setup = 'IxLoadExample' # each test setup has its own table schema inside the main PostgreSQL database. For SQLite, this maps directly to the filename.
	...
	logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', filename=os.path.join(dbpath,'Py27-genres-IxL.log'))
	...
	if len(sys.argv)==2:
		...
		resulttext = []
		resulttext.extend(my_test.csv_add_extract(os.path.join(testpath, 'HTTP_Client.csv'), testrun.get_empty_result(), {'blocknum': 2, 'headerline': 0, 'dataline': 3, 'columnssteady': [2], 'columnssteadyd': [110, 97], 'columnsmedian': [2,110]})) # <== add extracts of csvfile
		resulttext.extend(my_test.csv_add_extract(os.path.join(testpath, 'HTTP_Server.csv'), testrun.get_empty_result(), {'blocknum': 2, 'headerline': 0, 'dataline': 3, 'columnssteadyd': [25,31,30], 'columnsmedian': [25,31,30]})) # <== add extracts of csvfile
		if lcoslog.parse_loadlog_to_csv(os.path.join(testpath, 'LC1-COMlog.log'), os.path.join(testpath, 'load.csv')):
