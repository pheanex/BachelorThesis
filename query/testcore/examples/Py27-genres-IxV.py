...
from testcore.parseoutput import ixia
...
if __name__=='__main__':
	...
	test_setup = 'IxVPNExample' # each test setup has its own table schema inside the main PostgreSQL database. For SQLite, this maps directly to the filename.
	...
	logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', filename=os.path.join(dbpath,'Py27-genres-IxV.log'))
	...
	if len(sys.argv)==2:
		...
		if ixia.fix_ixvpn_csv(os.path.join(testpath, 'Throughput.csv'), os.path.join(testpath, 'ThroughputFix.csv')):
			resulttext = my_test.csv_add_extract(os.path.join(testpath, 'ThroughputFix.csv'), testrun.get_empty_result(),  {'columnsversus': [(1,0), (2,0), (3,0), (4,0)], 'columnsmeand': [2,4]}) # <== add extracts of csvfile
